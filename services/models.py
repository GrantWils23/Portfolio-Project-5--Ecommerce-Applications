...
class PaintService(models.Model):
    ...
    def calculate_estimate(self):
        ...
        self.base_estimate = (
            self.camo_pattern.base_price + self.weapon_system.base_price)
        self.save()
...
class TechService(models.Model):
    ...
    user_profile = models.ForeignKey(UserProfile, on_delete=models.SET_NULL,
                                     null=True, blank=True,
                                     related_name='techquote')
    full_name = models.CharField(max_length=50, null=False, blank=False)
    email = models.EmailField(max_length=254, null=False, blank=False)
    phone_number = PhoneNumberField(null=False, blank=False)
    date = models.DateTimeField(auto_now_add=True)
    weapon_platform = models.ForeignKey('WeaponPlatform', null=True,
                                        blank=False,
                                        on_delete=models.SET_NULL,
                                        related_name='wp')
    weapon_system = models.ForeignKey('WeaponSystem', null=True, blank=False,
    ...
    def calculate_estimate(self):
        """ return estimate price for admin """
        self.base_estimate = (
            self.weapon_platform.base_price + self.weapon_system.base_price)
        self.save()
